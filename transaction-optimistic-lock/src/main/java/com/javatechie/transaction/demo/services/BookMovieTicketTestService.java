...
public class BookMovieTicketTestService{
    ...
    private  static final Logger log = LoggerFactory.getLogger(BookMovieTicketTestService.class);
    ...
    public void bookTicketusingOptimisticLock(Long id) throws InterruptedException{


        Thread th1 = new Thread(() -> {
            try{
               log.info(Thread.currentThread().getName() + "trying to book the ticket using id : " +id);
                final Seat seat = bookMovieTicketService.bookMovieTicketusingOptimisticlock(id);
                log.info(Thread.currentThread().getName() + "Successfully book the ticket with id : {} , version : {}", id, seat.getVersion());
            }catch (Exception e) {
